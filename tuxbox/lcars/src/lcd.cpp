...
void lcd::setDirectPixel(int x, int y, int val)
{
	lcd_pixel pixel;
	pixel.x = x;
	pixel.y = y;
...
void lcd::	writeToFile(std::string filename)
{
...
void lcd::loadFromFile(std::string filename)
{
...
void lcd::loadFrom8bitFile(std::string filename)
{
	int f = open(filename.c_str(), O_RDONLY);
...
int lcd::loadFont(std::string filename)
{
...
void lcd::setTextSize(int size)
{
	current_textsize = size;
	//	error = FT_Set_Pixel_Sizes(face, 0, current_textsize);
}

void lcd::putText(int x, int y, int font, std::string text)
{
...
void lcd::draw_bitmap(character *ch, int x, int y)
{
	for (int y_pos = 0; y_pos < ch->rows; y_pos++)
	{
		int y_position = y_pos * ch->pitch;
