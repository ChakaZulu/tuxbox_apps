...
CMP3PlayerGui::CMP3PlayerGui()
{
   ...
   listmaxshow = max(1,(int)(height-info_height-title_height-theight-2*buttonHeight)/(fheight));
   ...
   y=(((g_settings.screen_EndY- g_settings.screen_StartY)-height)/ 2) + g_settings.screen_StartY;
	if(x+width+ConnectLineBox_Width > 720)
		x=720-width;
	y=(((g_settings.screen_EndY- g_settings.screen_StartY)-height) / 2) + g_settings.screen_StartY;
	if(y+height > 576)
		x=576-width;
...
/*			if ((int(selected)-int(listmaxshow))<0)
      ...
      else if(msg==CRCInput::RC_red && playlist.size() > 0)
      {
         ...
         if(selected > playlist.size()-1)
            selected = max((int)playlist.size()-1,0);
         update=true;
      }
      else if(msg==CRCInput::RC_green)
      {
         ...
         if(filebrowser->exec(Path))
         {
				...
				int radix=(int)sqrt(nrOfFiles-1)+1;
				int nr=0,v1,v2;
				for(; files != filebrowser->getSelectedFiles()->end();files++)
            {
					...
					v1=nr % radix;
					v2=nr / radix;
					progress->showLocalStatus((int)((float)v1/radix*100));
					progress->showGlobalStatus((int)((float)v2/radix*100));
