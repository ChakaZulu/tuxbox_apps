...
void CNeutrinoApp::InitMainSettings(CMenuWidget &mainSettings, CMenuWidget &audioSettings, CMenuWidget &networkSettings, CMenuWidget &colorSettings, CMenuWidget &keySettings, CMenuWidget &videoSettings)
{
	...
	mainSettings.addItem( new CMenuSeparator(20, CMenuSeparator::LINE | CMenuSeparator::STRING, "Info") );
		CStreamInfo* StreamInfo = new CStreamInfo();
	mainSettings.addItem( new CMenuForwarder("Stream Info", true, "", StreamInfo) );
...
void CNeutrinoApp::InitNetworkSettings(CMenuWidget &networkSettings)
{
	...
	CMenuOptionChooser* oj = new CMenuOptionChooser("setup network on startup", &settings.networkSetOnStartup, true );
		oj->addOption(0, "off");
		oj->addOption(1, "on");
