...
eDVB::eDVB()
{
	config.setName("/var/elitedvb/registry");
	if (config.open())
	{
		if (config.createNew())
		{
			mkdir("/var/elitedvb", 0777);
			if (config.createNew())
				qFatal("error while opening/creating registry - create /var/elitedvb");
		}
		if (config.open())
...
{
	FILE *f=fopen("/var/elitedvb/services", "wt");
	if (!f)
		qFatal("couldn't open servicefile - create /var/elitedvb!");
...
{
	FILE *f=fopen("/var/elitedvb/services", "rt");
	if (!f)
		return;
	char line[256];
...
void eDVB::saveBouquets()
{
	...
	FILE *f=fopen("/var/elitedvb/bouquets", "wt");
	if (!f)
		qFatal("couldn't open bouquetfile - create /var/elitedvb!");
...
{
	FILE *f=fopen("/var/elitedvb/bouquets", "rt");
	if (!f)
		return;
	char line[256];
