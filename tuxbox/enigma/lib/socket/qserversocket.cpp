/* small change by tmbinc: setAddressReusable(true); */
/****************************************************************************
** $Id: qserversocket.cpp,v 1.1 2002/06/03 11:37:29 Ghostrider Exp $
**
...
void QServerSocket::init( const QHostAddress & address, Q_UINT16 port, int backlog )
{
    ...
    if ( d->s->bind( address, port ) && d->s->listen( backlog ) )
    {
			d->n = new eSocketNotifier( eApp, d->s->socket(), eSocketNotifier::http );
			CONNECT( d->n->activated, QServerSocket::incomingConnection );
    }
		else
		{
			eWarning( "QServerSocket: failed to bind or listen to the socket" );
			delete d->s;
			d->s = 0;
    }
}
...
{
	int fd = d->s->accept();
	eDebug("INCOMMING CONNECTION requested Port = %i", fd);
  if ( fd >= 0 )
			newConnection( fd );
}
