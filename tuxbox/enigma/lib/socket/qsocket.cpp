/****************************************************************************
** $Id: qsocket.cpp,v 1.1 2002/06/03 11:37:29 Ghostrider Exp $
**
...
public:
   ...
   ~QSocketPrivate();
    ...
    eSocketNotifier    *rsn, *wsn;		// socket notifiers
...
void QSocket::tryConnecting()
{
	...
	if ( d->socket->connect( l[0], d->port ) == FALSE ) {
	    ...
	    d->state = Idle;
	    /*emit*/ error_( ErrConnectionRefused );
	    return;
	}
#if defined(QSOCKET_DEBUG)
...
void QSocket::flush()
{
	...
	if ( (int)a->size() - d->windex < 1460 ) {
	    ...
	    if ( d->wsn )
		d->wsn->stop(); // the QSocketNotifier documentation says so
	} else {
...
void QSocket::sn_read(int)
{
	...
	if ( nread == 0 ) {			// really closed
		...
		if (d->rsn)
		    d->rsn->stop();
		/*emit*/ error_( ErrSocketRead );	// socket close error
		alreadyCalled = FALSE;
		return;
	    }
	...
	if ( nread < 0 ) {
	    ...
	    if (d->rsn)
		d->rsn->stop();
	    /*emit*/ error_( ErrSocketRead );	// socket read error
	    alreadyCalled = FALSE;
	    return;
	}
...
void QSocket::setSocketIntern( int socket )
{
    ...
    d->state = Idle;
    d->rsn = new eSocketNotifier( eApp, d->socket->socket(), eSocketNotifier::http );

    d->wsn = new eSocketNotifier( eApp, d->socket->socket(), eSocketNotifier::Write );

		CONNECT( d->rsn->activated, QSocket::sn_read );
    d->rsn->stop();
		CONNECT( d->wsn->activated, QSocket::sn_write );
    d->wsn->stop();
