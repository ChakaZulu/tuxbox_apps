...
extern eString getZapContent2(eString mode, eString path, int depth, bool addEPG, bool sortList);
...
static eString changeTimerEvent(eString request, eString dirpath, eString opts, eHTTPConnection *content)
{
	...
	eDebug("[ENIGMA_DYN] changeTimerEvent %s.%s.%s - %s:%s, %s.%s.%s - %s:%s", sday.c_str(), smonth.c_str(), syear.c_str(), shour.c_str(), smin.c_str(), eday.c_str(), emonth.c_str(), eyear.c_str(), ehour.c_str(), emin.c_str());
