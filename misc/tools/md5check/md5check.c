...
const unsigned char avia500[] = { 0xfe, 0xce, 0x1d, 0x33, 0x24, 0xe0, 0x91, 0x7b, 0x92, 0x1d, 0x81, 0x44, 0x90, 0xd8, 0xa8, 0x24 }; 
const unsigned char avia600[] = { 0xda, 0x49, 0x21, 0x46, 0xba, 0x7e, 0x17, 0x78, 0x83, 0xfe, 0xad, 0xaa, 0x0c, 0xf8, 0x9a, 0xa5 }; 
...
char *cam_alpha_file = "/var/ucodes/cam-alpha.bin";
char *avia500_file = "/var/ucodes/avia500.ux";
char *avia600_file = "/var/ucodes/avia600.ux";
char *ucode_file = "/var/ucodes/ucode.bin";
...
md5_file(filename, 1, &md5buffer);
	...
	int i;
	int check=0;
	for(i=0;i<16;i++) if(avia500[i]==md5buffer[i]) check++;
	if (check==16) 
		printf("avia500.ux ist OK\n");
	else
                {
		for (cnt = 0; cnt < 16; ++cnt)
		printf ("%02x\n", md5buffer[cnt]);
		printf("avia500.ux ist nicht OK\n");
		}
	}
...
md5_file(filename, 1, &md5buffer);
        ...
        int i;
        int check=0;
        for(i=0;i<16;i++) if(avia600[i]==md5buffer[i]) check++;
        if (check==16)
                printf("avia600.ux ist OK\n");
        else
		{
                for (cnt = 0; cnt < 16; ++cnt)
                printf ("%02x\n", md5buffer[cnt]);
                printf("avia600.ux ist NICHT OK\n");
		}
        }
...
md5_file(filename, 1, &md5buffer);
        ...
        if (check==16)
                printf("cam-alpha.bin = 01.01.004D\n");
